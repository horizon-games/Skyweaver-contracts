...
describe('ConquestV3', () => {
  ...
  let conquestV3Abstract: AbstractContract
  let oldConquestAbstract: AbstractContract
  let oldConquestV2Abstract: AbstractContract
  ...
  let oldConquestV2Contract: ConquestV2
  let conquestV3Contract: ConquestV3
  let silverFactory: RewardFactory
  let goldFactory: RewardFactory

  // Factory manager
  let userOldConquestContract: Conquest
  let userOldConquestV2Contract: ConquestV2
  let userConquestV3Contract: ConquestV3
  ...
  before(async () => {
    ...
    oldConquestV2Abstract = await AbstractContract.fromArtifactName('ConquestV2')
    conquestV3Abstract = await AbstractContract.fromArtifactName('ConquestV3')
  ...
  beforeEach(async () => {
    ...
    // Deploy conquest V2
    oldConquestV2Contract = await oldConquestV2Abstract.deploy(ownerWallet, [
      ownerAddress,
    ...
    userOldConquestV2Contract = await oldConquestV2Contract.connect(userSigner) as ConquestV2

    // Deploy conquest V3
    conquestV3Contract = await conquestV3Abstract.deploy(ownerWallet, [
      ownerAddress,
      oldConquestV2Contract.address,
      skyweaverAssetsContract.address,
      silverFactory.address,
      goldFactory.address,
      ticketID
    ]) as ConquestV3
    userConquestV3Contract = await conquestV3Contract.connect(userSigner) as ConquestV3

    // Allow conquest to print cards in the factories
    await silverFactory.assignOwnership(conquestV3Contract.address, 1)
    await goldFactory.assignOwnership(conquestV3Contract.address, 1)

    // Assing vars
    factory = conquestV3Contract.address
    ...
    context(condition as string, () => {
      ...
      beforeEach( async () => {
        if (condition == conditions[0]) {
          ...
          await userSkyweaverAssetContract.safeBatchTransferFrom(userAddress, oldConquestV2Contract.address, [ticketID], [100], [], TX_PARAM)
          await oldConquestV2Contract.exitConquest(userAddress, 3, [], [])
        }
      })
      ...
      describe('Getter functions', () => {
        describe('skyweaverAssets() function', () => {
          it('should return Factory manager contract address', async () => {
            const manager = await conquestV3Contract.skyweaverAssets()
        ...
        describe('supportsInterface()', () => {
          it('should return true for 0x01ffc9a7 (ERC165)', async () => {
            const support = await conquestV3Contract.supportsInterface('0x01ffc9a7')
            expect(support).to.be.eql(true)
          })

          it('should return true for 0x4e2312e0 (ERC1155Receiver)', async () => {
            const support = await conquestV3Contract.supportsInterface('0x4e2312e0')
            expect(support).to.be.eql(true)
          })
        })
      })
      ...
      describe('Conquest', () => {
        ...
        context('Using safeBatchTransferFrom', () => {
          ...
          it('should REVERT if token address is not ticket', async () => {
            ...
            await expect(tx).to.be.rejectedWith(RevertError("ConquestV3#entry: INVALID_ENTRY_TOKEN_ADDRESS"))
          })
          ...
          it('should REVERT if ID is not ticket', async () => {
            ...
            await expect(tx).to.be.rejectedWith(RevertError("ConquestV3#entry: INVALID_ENTRY_TOKEN_ID"))
          })
          ...
          it('should REVERT if sent more than 1 ticket', async () => {
            ...
            await expect(tx).to.be.rejectedWith(RevertError("ConquestV3#entry: INVALID_ENTRY_TOKEN_AMOUNT"))
          })
          ...
          it('should REVERT if caller is already in a conquest', async () => {
            ...
            await expect(tx2).to.be.rejectedWith(RevertError("ConquestV3#entry: PLAYER_ALREADY_IN_CONQUEST"))
          })
        })
        ...
        context('Using safeTransferFrom', () => {
          ...
          it('should REVERT if caller is already in a conquest', async () => {
            ...
            const tx = userSkyweaverAssetContract.safeTransferFrom(userAddress, factory, ticketID, amount, [], TX_PARAM)
            await expect(tx).to.be.rejectedWith(RevertError("ConquestV3#entry: PLAYER_ALREADY_IN_CONQUEST"))
          })
        })
        ...
        context('When ticket was sent to factory', () => {
          ...
          it('should set user as being in conquest', async () => {
            let value = await conquestV3Contract.isActiveConquest(userAddress)
            expect(value).to.be.eql(true)
          })

          it('should update user conquest count', async () => {
            let value = await conquestV3Contract.conquestsEntered(userAddress)
          ...
          it('should emit ConquestEntered event', async () => {
            ...
            filterFromOperatorContract = conquestV3Contract.filters.ConquestEntered(null, null);
            ...
            expect(logs[0].topics[0]).to.be.eql(conquestV3Contract.interface.getEventTopic(conquestV3Contract.interface.events["ConquestEntered(address,uint256)"]))
          })
          ...
          describe('ConquestEntered Event', () => {
            ...
            beforeEach(async () => {
              ...
              args = conquestV3Contract.interface.decodeEventLog(conquestV3Contract.interface.events["ConquestEntered(address,uint256)"],logs[0].data, logs[0].topics)
            })
      ...
      describe('exitConquest()', () => {
        ...
        it('should REVERT if caller is not in conquest', async () => {
          const tx = conquestV3Contract.exitConquest(userAddress, expectedNonce, silver_ids, gold_ids)
          await expect(tx).to.be.rejectedWith(RevertError("ConquestV3#exitConquest: USER_IS_NOT_IN_CONQUEST"))
        })
          ...
          conditions.forEach(function(condition) {
            context(condition[2] as string, () => {
              ...
              it('should PASS if caller is owner', async () => {
                const tx = conquestV3Contract.exitConquest(userAddress, expectedNonce, silverRewardIds, goldRewardIds)
                await expect(tx).to.be.fulfilled
              })

              it('should PASS if rewards are duplicates', async () => {
                const tx = conquestV3Contract.exitConquest(userAddress, expectedNonce, [23, 23], [])
                await expect(tx).to.be.fulfilled
              })

              it('should PASS if rewards are unsorted', async () => {
                const tx = conquestV3Contract.exitConquest(userAddress, expectedNonce, [7, 1], [])
                await expect(tx).to.be.fulfilled
              })


              it('should REVERT if silver reward is not silver', async () => {
                const tx = conquestV3Contract.exitConquest(userAddress, expectedNonce, [GOLD_SPACE], [])
              ...
              it('should REVERT if caller is not owner', async () => {
                const tx = userConquestV3Contract.exitConquest(userAddress, expectedNonce, silverRewardIds, goldRewardIds)
              ...
              it('should REVERT if trying to mint more than possible amount', async () => {
                const tx4 = conquestV3Contract.exitConquest(userAddress, expectedNonce, [1, 2, 3], [])
                await expect(tx4).to.be.rejectedWith(RevertError("ConquestV3#exitConquest: INVALID_REWARDS"))

                const tx1 = conquestV3Contract.exitConquest(userAddress, expectedNonce, [1], [GOLD_SPACE.add(1), GOLD_SPACE.add(2)])
                await expect(tx1).to.be.rejectedWith(RevertError("ConquestV3#exitConquest: INVALID_REWARDS"))

                const tx2 = conquestV3Contract.exitConquest(userAddress, expectedNonce, [1, 2], [GOLD_SPACE.add(1)])
                await expect(tx2).to.be.rejectedWith(RevertError("ConquestV3#exitConquest: INVALID_REWARDS"))

                const tx3 = conquestV3Contract.exitConquest(userAddress, expectedNonce, [], [GOLD_SPACE.add(1)])
                await expect(tx3).to.be.rejectedWith(RevertError("ConquestV3#exitConquest: INVALID_REWARDS"))

                const tx5 = conquestV3Contract.exitConquest(userAddress, expectedNonce, [1], [GOLD_SPACE.add(1)])
              ...
              describe('mintGold()', () => {
                ...
                it('should REVERT if gold reward is not gold', async () => {
                  await conquestV3Contract.exitConquest(userAddress, expectedNonce, [1], [GOLD_SPACE.sub(1)])
                  const tx = conquestV3Contract.mintGold(userAddress, expectedNonce)
                  await expect(tx).to.be.rejectedWith(RevertError('SkyweaverAssets#_validateMints: ID_OUT_OF_RANGE'))
                })

                it('should REVERT if conquest didnt reserve a gold', async () => {
                  await conquestV3Contract.exitConquest(userAddress, expectedNonce, [1], [])
                  const tx = conquestV3Contract.mintGold(userAddress, expectedNonce)
                  await expect(tx).to.be.rejectedWith(RevertError('ConquestV3#mintGold: NO_GOLD_TO_BE_MINTED_FOR_THIS_CONQUEST'))
                })

                it('should REVERT if conquest is not exited', async () => {
                  const tx = conquestV3Contract.mintGold(userAddress, expectedNonce)
                  await expect(tx).to.be.rejectedWith(RevertError('ConquestV3#mintGold: NO_GOLD_TO_BE_MINTED_FOR_THIS_CONQUEST'))
                })

                it('should REVERT if conquest is in the future', async () => {
                  const tx = conquestV3Contract.mintGold(userAddress, expectedNonce+1)
                  await expect(tx).to.be.rejectedWith(RevertError('ConquestV3#mintGold: NO_GOLD_TO_BE_MINTED_FOR_THIS_CONQUEST'))
                })

                it('should PASS if conquest reserved a gold', async () => {
                  await conquestV3Contract.exitConquest(userAddress, expectedNonce, silverRewardIds, goldRewardIds)
                  const tx = conquestV3Contract.mintGold(userAddress, expectedNonce)
                  await expect(tx).to.be.fulfilled
                })

                it('should not have the gold as mintable yet', async () => {
                  const goldPending = await conquestV3Contract.pendingEntered(userAddress, expectedNonce)
                  expect(goldPending).to.be.eql(BigNumber.from(0))
                })
              })
              ...
              context('When exitConquest was called', () => {
                ...
                it('should set the gold as mintable', async () => {
                  const goldPending = await conquestV3Contract.pendingEntered(userAddress, expectedNonce)
                  if (condition == conditions[1]) {
                    expect(goldPending).to.be.eql(BigNumber.from(0))
                  } else {
                    expect(goldPending).to.be.eql(BigNumber.from(goldRewardIds[0]))
                  }
                })

                describe('mintGold()', () => {
                  context("When gold was minted", () => {
                    before(async function() {
                      if (condition == conditions[1]) {
                        this.test!.parent!.pending = true
                        this.skip()
                    ...
                    beforeEach(async () => {
                      await conquestV3Contract.mintGold(userAddress, expectedNonce)
                    })

                    it('should set the gold as not mintable', async () => {
                      const goldPending = await conquestV3Contract.pendingEntered(userAddress, expectedNonce)
                      expect(goldPending).to.be.eql(BigNumber.from(0))
                    })
  ...
  describe('when old conquest has a state', () => {
    ...
    it('exit should PASS if old conquest is in conquest and conquest is not synced', async () => {
      ...
      const tx = conquestV3Contract.exitConquest(userAddress, 1, [], [])
      await expect(tx).to.be.fulfilled
    })
    ...
    it('isActiveConquest() should return old status if conquest is not synced', async () => {
      const status0 = await conquestV3Contract.isActiveConquest(userAddress)
      await userSkyweaverAssetContract.safeTransferFrom(userAddress, oldConquestContract.address, ticketID, amount, [], TX_PARAM)
      const status1 = await conquestV3Contract.isActiveConquest(userAddress)
    ...
    it('isActiveConquest() should return new status if conquest is synced', async () => {
      ...
      const newStatus0 = await conquestV3Contract.isActiveConquest(userAddress)

      await conquestV3Contract.exitConquest(userAddress, 1, [], [])
      const oldStatus1 = await oldConquestContract.isActiveConquest(userAddress)
      const newStatus1 = await conquestV3Contract.isActiveConquest(userAddress)
    ...
    it('conquestsEntered() should return old status if conquest is not synced', async () => {
      const status0 = await conquestV3Contract.conquestsEntered(userAddress)
      await userSkyweaverAssetContract.safeTransferFrom(userAddress, oldConquestContract.address, ticketID, amount, [], TX_PARAM)
      const status1 = await conquestV3Contract.conquestsEntered(userAddress)
    ...
    it('conquestsEntered() should return new status if conquest is synced', async () => {
      ...
      const newStatus0 = await conquestV3Contract.conquestsEntered(userAddress)

      await userSkyweaverAssetContract.safeTransferFrom(userAddress, factory, ticketID, amount, [], TX_PARAM)
      const oldStatus1 = await oldConquestContract.conquestsEntered(userAddress)
      const newStatus1 = await conquestV3Contract.conquestsEntered(userAddress)
